...
def spatiotemporal_summary(posttimes,coords,probs):
    #time_coords_clean = [x for x in time_coords if not np.isnan(x).any()]
    if len(posttimes)>1:
        dind = pd.to_datetime(posttimes)
        df_coords = Series(coords, index=dind)
        df_probs = Series(probs, index=dind)
        df = pd.concat([df_coords,df_probs],axis=1)
